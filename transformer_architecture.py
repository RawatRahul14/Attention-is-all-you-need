...
Original file is located at
    https://colab.research.google.com/drive/1m8_X2haZx5M_rfIVPJ-7T_vEJVUdwcLm
"""
...
class EncoderLayer(nn.Module):
  def __init__(self, d_model, ffn_hidden, num_heads, drop_prob):
    ...
    self.ffn = PositionWiseForward(d_model = d_model, hidden = ffn_hidden, drop_prob = drop_prob)
...
class DecoderLayer(nn.Module):
  ...
  def forward(self, x, y, self_attention_mask, cross_attention_mask):
    ...
    y = self.dropout1(y)
    y = self.layer_norm1(_y + y)

    _y = y.clone()
    y = self.encoder_decoder_attention(x, y, mask = cross_attention_mask)
    y = self.dropout2(y)
    y = self.layer_norm2(_y + y)

    _y = y.clone()
    y = self.ffn(y)
    y = self.dropout3(y)
    y = self.layer_norm3(_y + y)
    return y
